...
- Evaluate the datasets suitability for machine learning applications using quantitative metrics
...
### Number of locations/countries per AEZ  

```{r}
# AEZ summary: number of countries, locations, and observations per AEZ
aez_summary <- data %>%
  filter(!is.na(AEZ)) %>%
  group_by(AEZ) %>%
  summarise(
...
# Display as table
aez_summary %>%
  kable(caption = "Summary of Observations by Agroecological Zone (AEZ)") %>%
...
head(yearly_summary, 50) %>%
  kable(caption = "Across the years: Observations and Yield Statistics")
```
